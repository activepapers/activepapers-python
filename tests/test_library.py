...
def test_figshare_download():
    with tempdir.TempDir() as t:
        ...
        assert paper.code_group['python-packages/immutable/__init__'].attrs['ACTIVE_PAPER_DATATYPE'] == 'module'
        paper.close()
...
def test_zenodo_download():
    with tempdir.TempDir() as t:
        ...
        paper = ActivePaper(local_name)
        assert paper.code_group['python-packages/mosaic/__init__'].attrs['ACTIVE_PAPER_DATATYPE'] == 'module'
        paper.close()
